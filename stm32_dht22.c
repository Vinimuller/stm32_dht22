...
uint32_t DHT22_GetReadings(void) {
	...
	DHT22_SDA_PORT->MODER	|= (uint32_t)(0x01 << DHT22_SDA_PIN * 2);
	DHT22_SDA_PORT->ODR		&= ~(0x01 << DHT22_SDA_PIN); // Pull down SDA (Bit_SET)
	Delay_ms(2);

	DHT22_SDA_PORT->MODER	&= ~(0x11 << DHT22_SDA_PIN * 2);
	DHT22_SDA_PORT->PUPDR	|= (0x01 << DHT22_SDA_PIN * 2);
	...
	while (i < 40) {
		...
		if (wait > 15) {
			...
			wait = 0;
			while ((DHT22_SDA_PORT->IDR & (1 << DHT22_SDA_PIN)) && (wait++ < 25)) Delay_us(5);
			dht22Data.bits[i] = (wait < 25) ? wait : 0xffff;
		}

		i++;
	}

	for (i = 0; i < 40; i++) if (dht22Data.bits[i] == 0xffff) return DHT22_RCV_RCV_TIMEOUT;
